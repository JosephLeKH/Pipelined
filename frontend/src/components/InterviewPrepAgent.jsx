...
function DoneView({ briefing, onReset }) {
...
export function InterviewPrepAgent({ applicationId, briefing: appBriefing, generatedAt }) {
  const { status, progressSteps, briefing, errorMessage, start, reset, STATUS } =
    useInterviewPrep(applicationId);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {status === STATUS.IDLE && <IdleView onStart={start} />}
      {status === STATUS.RUNNING && <RunningView steps={progressSteps} />}
      {status === STATUS.ERROR && <ErrorView message={errorMessage} onRetry={start} />}
      {status === STATUS.DONE && briefing && <DoneView briefing={briefing} onReset={reset} />}
    </div>
  );
}
